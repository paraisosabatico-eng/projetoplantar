...
let idx1 = html.indexOf(cssStart);
let idx2 = html.indexOf(cssEndPattern);
if(idx1 !== -1 && idx2 !== -1) {
    html = html.substring(0, idx1) + cssNew + html.substring(idx2);
} else {
    console.error("Failed to find CSS block");
}

// 2. Replace HTML
const htmlStart = '    <!-- Dobra 3 -->';
const htmlEndPattern = '    <!-- ══════════════════════════════════════════════════\n         DOBRA 4 — POR QUE O PROJETO EXISTE';
const htmlNew = `    <!-- Dobra 3 -->
...
let idx3 = html.indexOf(htmlStart);
let idx4 = html.indexOf(htmlEndPattern);
if (idx3 !== -1 && idx4 !== -1) {
    html = html.substring(0, idx3) + htmlNew + html.substring(idx4);
} else {
    console.error("Failed to find HTML block");
}

// 3. Replace GSAP
const gsapStart = '            // Animação Bento Box Dobra 3';
const gsapEndPattern = '            // Animação Dobra 5';
const gsapNew = `            // Parallax e Animação Dobra 3
...
let idx5 = html.indexOf(gsapStart);
let idx6 = html.indexOf(gsapEndPattern);
if (idx5 !== -1 && idx6 !== -1) {
    html = html.substring(0, idx5) + gsapNew + html.substring(idx6);
} else {
    console.error("Failed to find GSAP block");
}
